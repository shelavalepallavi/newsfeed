...
const YouFollow = () => {
  ...
  const youFollowShow = showYou ? youFollow : youFollow.slice(0, 4);
    ...
    <div className='mt-1 mb-2 bg-gray-100 dark:bg-gray-800 rounded-md pt-1 pb-1 relative'>
      <div>
        ...
        <div className='flex flex-wrap'>
          {youFollowShow.map((fol, i) => (
            <div className='flex items-start mb-3' key={i}>
              <img 
                src={fol.image} 
                alt="" 
                className='w-10 h-10 rounded-full mr-2 ml-2 object-cover' 
              />
            </div>
          ))}
        </div>
      ...
      {youFollow.length > 4 && (
        <div className='flex justify-end mr-4 text-blue-700 dark:text-blue-400 cursor-pointer'>
          <button onClick={(e) => {
            e.stopPropagation();
            setShowYou((prev) => !prev);
          }}>
            {showYou ? 'Show Less' : 'Show All'}
          </button>
        </div>
      )}
    </div>
